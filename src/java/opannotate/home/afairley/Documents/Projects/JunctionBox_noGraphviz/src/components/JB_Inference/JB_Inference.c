               ...
               :}
               :
               :void BNet_InitializePotentialsLog(BNet *this){ /* BNet_InitializePotentialsLog total:      4  0.3295 */
               :  //initialize seperator potentials
               :  EdgeItr *eitr = Graph_GetEdgeItr(this->elimTree);
               :  Edge *curEdge;
               :  int i = 0,j;
               ...
               :  for(i =0; i < this->numTables; i++){
               :    Pot *curTable = (Pot*)this->tables[i]->pot;
     2  0.1647 :    VariableSet *curVbls = curTable->vbls;
     1  0.0824 :    for(j = 0; j < this->numCliques; j++){
               :      Pot *curClique = *(this->cliquePots[j]);
     1  0.0824 :      if(VariableSet_ContainedIn(curVbls,curClique->vbls)){
               :	curVertex = (Vertex*)curClique->owner;
               :	MulInd *mi = MulMemo_GetMargInd(curVertex->margMemo,
               :					curClique->vbls,
               :					curVbls);
               ...
               :
               :
               :char *Pot_ToString(Pot *this){ /* Pot_ToString total:      3  0.2471 */
               :  int i;
     1  0.0824 :  char *msg = jballoc( strlen(this->label) + 1 + (32 * this->tableSize));
               :  msg[0] = '\0';
               :  strcat(msg, this->label);
               :  strcat(msg, " ");
               :  char buf[32];
               :  for(i = 0; i < this->tableSize; i++){
     1  0.0824 :    sprintf(buf ,"%f ", this->valueData.tabData[i]);
     1  0.0824 :    strcat(msg, buf);
               :  }
               :  return msg;
               :}
               :
               ...
               :}
               :
               :void Print_BNet(BNet *bn){ /* Print_BNet total:      2  0.1647 */
               :  int i;
               :  printf("\n************************************\n");
     ...
     1  0.0824 :    printf("\n %s \n",Pot_ToString(*(bn->seperatorPots[i])));
               ...
               :}
/* 
 * Total samples for file : "/home/afairley/Documents/Projects/JunctionBox_noGraphviz/src/components/JB_Inference/JB_Inference.c"
 * 
 *      9  0.7414
 */
