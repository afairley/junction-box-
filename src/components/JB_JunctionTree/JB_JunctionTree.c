...
int VertexOrdering_GetVertexRankByLabel(VertexOrdering *this, char *c){
  int ret;
  for(ret = 0; ret < this->size; ret++){
    if(strcmp( Vertex_GetLabel(this->vertices[ret]),c) == 0)
       return ret;
  }
  return -1;
}
...
char *ElimSet_GetLabel(ElimSet *this){
  int i, length = 0;
  char *ret;
...
List *ElimSet_GetVertexList(ElimSet *this, Graph *g){
  int i;
...
ElimSet *ElimSet_Intersect(ElimSet *this, ElimSet *other){
  ElimSet *ret = (ElimSet*)jballoc(sizeof(ElimSet));
...
int ECCollection_DefEdgeComp(const void *ec1, const void *ec2){
  EdgeCandidate *edgec1 = *(EdgeCandidate**)ec1, *edgec2 =*(EdgeCandidate**)ec2;
...
ECCollection *ECCollection_Create(VertexOrdering *o, int (*cmp)(const void *, const void*) ){
  ...
  for(i = 0, index = 0; i < o->size; i++){
    ...
    for(j = i + 1; j < o->size; j++){
      sink = o->vertices[j];
      ret->edgeCs[index++] = EdgeCandidate_Create(src,sink);
    }
  }
...
ElimSetCollection *GatherElimSets(Graph *moral, 
  ...
  for(i = o->size -1; i > -1; i--){
    Vertex *current = Graph_GetVertex(moral,
				      ...
				      );//Vertices in o are not from moral!
    Vertex *nghbr;
